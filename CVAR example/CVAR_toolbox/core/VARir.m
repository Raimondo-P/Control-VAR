...
%       - unit_norm: if 1, impulse responses are unit normalized (impact of 
%                    shock on itself = 1); if 0, no normalization
% -------------------------------------------------------------------------
% OUTPUT
...
% Set default for unit_norm if not provided
if ~isfield(VARopt,'unit_norm')
    VARopt.unit_norm = 0;  % default: no normalization
end
...
for mm=1:nvar
    ...
    if unit_norm == 1
        norm_factor = response(mm,1);  % impact response of shock mm on itself
        if abs(norm_factor) > 1e-10  % avoid division by zero
