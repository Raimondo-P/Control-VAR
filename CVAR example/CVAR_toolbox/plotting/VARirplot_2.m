function VARirplot_2(IR,VARopt,INF,SUP)
% =======================================================================
...
FigSize(VARopt.FigSize(1),VARopt.FigSize(2))
    ...
    for ii=1:3
        subplot(col,1,ii);
        plot(steps,IR(:,ii,1),'LineStyle','-','Color','k','LineWidth',2,'Marker',SwatheOpt.marker); hold on
        if exist('INF','var') && exist('SUP','var')
            PlotSwathe(IR(:,ii,1),[INF(:,ii,1) SUP(:,ii,1)],SwatheOpt); hold on;
        end
        plot(x_axis,'--k','LineWidth',0.5); hold on
        xlim([1 nsteps]);
        title([vnames{ii} ' to ' snames{1}], 'FontWeight','bold','FontSize',10); 
        set(gca, 'Layer', 'top');
    end
